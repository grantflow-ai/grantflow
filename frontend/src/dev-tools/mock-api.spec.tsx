...
describe("Mock API Core Integration", () => {
	...
	describe("Mock API Client Initialization", () => {
		...
		it("should register mock handlers on initialization", async () => {
			...
			expect(mockRegister).toHaveBeenCalledWith("/auth/login", expect.any(Function));
			expect(mockRegister).toHaveBeenCalledWith("/projects", expect.any(Function));
			expect(mockRegister).toHaveBeenCalledWith("/projects/:project_id/applications", expect.any(Function));
		});
