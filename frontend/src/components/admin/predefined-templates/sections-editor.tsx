...
export function PredefinedTemplateSectionsEditor({ onChange, sections }: SectionsEditorProps) {
	...
	const renderSection = (section: EditableGrantSection, isSubsection = false): React.JSX.Element => {
		...
		const lengthLabel = (() => {
			const { type, value } = section.length_constraint;
			if (type === "words") {
							...
							<div className="px-6 py-3">
								<div className="flex flex-col space-y-4 2xl:space-y-5">
									...
									<div className="space-y-2 2xl:space-y-3">
										...
										<p className="text-app-gray-600 text-base font-normal leading-tight">
											Your AI assistant will follow this instruction. Modify the prompt to reflect
											your topic and goals for a more relevant draft.
										</p>
										<Label className="block text-start text-xs font-light text-input-label mb-1">
											AI Prompt
										</Label>
										<AppTextarea
									...
									<div className="space-y-2">
										...
										<p className="text-app-gray-600 text-base font-normal leading-tight">
											Is this section the main Research Plan? Each application can designate one
											section as the official Research Plan.
										</p>
									</div>
