...
describe.sequential("GrantCard", () => {
	...
	it("displays grant title", () => {
		const grant = GrantFactory.build({ title: "Test Grant Title" });
		render(<GrantCard grant={grant} />);
	...
	it("displays activity code badge when present", () => {
		const grant = GrantFactory.build({ activity_code: "R01" });
		render(<GrantCard grant={grant} />);
	...
	it("displays grant description when present", () => {
		const grant = GrantFactory.build({ description: "This is a test grant description." });
		render(<GrantCard grant={grant} />);
	...
	it("does not display description when not present", () => {
		const grant = GrantFactory.build({ description: undefined });
		render(<GrantCard grant={grant} />);
	...
	it("displays eligibility info when present", () => {
		const grant = GrantFactory.build({ eligibility: "Must be US citizen" });
		render(<GrantCard grant={grant} />);
	...
	it("does not display eligibility info when not present", () => {
		const grant = GrantFactory.build({ eligibility: undefined });
		render(<GrantCard grant={grant} />);
	...
	it("displays View Details link when URL is present", () => {
		const grant = GrantFactory.build({ url: "https://grants.nih.gov/test" });
		render(<GrantCard grant={grant} />);
	...
	describe("Deadline formatting", () => {
		...
		it("displays 'Ongoing' when deadline is not provided", () => {
			const grant = GrantFactory.build({ deadline: undefined });
			render(<GrantCard grant={grant} />);
		...
		it("displays 'Today' when deadline is today", () => {
			const grant = GrantFactory.build({ deadline: "2024-01-15T23:59:59Z" });
			render(<GrantCard grant={grant} />);
		...
		it("displays 'Tomorrow' when deadline is tomorrow", () => {
			const grant = GrantFactory.build({ deadline: "2024-01-16T23:59:59Z" });
			render(<GrantCard grant={grant} />);
		...
		it("displays formatted date when deadline is far in future", () => {
			const grant = GrantFactory.build({ deadline: "2024-06-15T23:59:59Z" });
			render(<GrantCard grant={grant} />);
		...
		it("displays 'Expired' when deadline is in the past", () => {
			const grant = GrantFactory.build({ deadline: "2024-01-10T23:59:59Z" });
			render(<GrantCard grant={grant} />);
	...
	describe("Amount formatting", () => {
		it("displays amount range when both min and max are provided", () => {
			const grant = GrantFactory.build({ amount_max: 500_000, amount_min: 100_000 });
			render(<GrantCard grant={grant} />);
		...
		it("displays minimum amount with plus when only min is provided", () => {
			const grant = GrantFactory.build({ amount_max: undefined, amount_min: 250_000 });
			render(<GrantCard grant={grant} />);
		...
		it("displays maximum amount with 'Up to' when only max is provided", () => {
			const grant = GrantFactory.build({ amount_max: 750_000, amount_min: undefined });
			render(<GrantCard grant={grant} />);
		...
		it("displays 'Amount varies' when neither min nor max are provided", () => {
			const grant = GrantFactory.build({ amount_max: undefined, amount_min: undefined });
			render(<GrantCard grant={grant} />);
		...
		it("formats amounts with thousand separators", () => {
			const grant = GrantFactory.build({ amount_max: 2_500_000, amount_min: 1_000_000 });
			render(<GrantCard grant={grant} />);
	...
	describe("Content truncation", () => {
		it("applies line clamp to description", () => {
			...
			const grant = GrantFactory.build({ description: longDescription });
			render(<GrantCard grant={grant} />);
