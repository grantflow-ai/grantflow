...
export function Notification({}: NotificationProps) {
  return (
    ...
    {notification.map((item)=>(
        <DropdownMenuItem data-testid={`notification-item-${item.id}`} key={item.id} className={`p-4 font-normal gap-1 text-base text-gray-700 flex flex-col items-start data-[highlighted]:bg-preview-bg data-[highlighted]:text-gray-700  rounded-none border border-gray-200 cursor-pointer`}>
            <div className="flex items-center justify-between w-full">
                <div className={` size-3 rounded-full ${item.dotColor}`} data-testid={`notification-dot-${item.id}`}></div>
                <div data-testid={`notification-close-${item.id}`}>
                    <X className="size-3 text-gray-700" />
                </div>
            </div>
            <p className="font-semibold text-base text-black" data-testid={`notification-title-${item.id}`}>
                {item.title}
            </p>
            <p className="text-sm text-gray-600 font-normal" data-testid={`notification-description-${item.id}`}>
                {item.description}
            </p>
        </DropdownMenuItem>
    ))}
