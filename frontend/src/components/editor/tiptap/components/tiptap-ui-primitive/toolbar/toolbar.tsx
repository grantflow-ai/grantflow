...
import "@/components/editor/tiptap/components/tiptap-ui-primitive/toolbar/toolbar.css";
...
const mergeRefs = <T,>(refs: (null | React.Ref<T> | React.RefObject<T> | undefined)[]): React.RefCallback<T> => {
	return (value) => {
		refs.forEach((ref) => {
			if (typeof ref === "function") {
				ref(value);
			} else if (ref != null) {
				(ref as React.MutableRefObject<null | T>).current = value;
			}
		});
	};
};
	...
	React.useEffect(() => {
		...
		const handleKeyDown = (e: KeyboardEvent) => {
			...
			const keyActions: Record<string, () => void> = {
				...
				Home: () => {
					navigateToIndex(e, 0, focusableElements);
				},
			};
...
const useToolbarVisibility = (ref: React.RefObject<HTMLDivElement | null>): boolean => {
	...
	const checkVisibility = React.useCallback(() => {
		...
		const hasVisibleChildren = [...toolbar.children].some((child) => {
...
const useGroupVisibility = (ref: React.RefObject<HTMLDivElement | null>): boolean => {
	...
	const checkVisibility = React.useCallback(() => {
		...
		const hasVisibleChildren = [...group.children].some((child) => {
			if (!(child instanceof HTMLElement)) return false;
			return true;
		});
...
export const ToolbarGroup = React.forwardRef<HTMLDivElement, BaseProps>(({ children, className, ...props }, ref) => {
	...
	return (
		<div className={`tiptap-toolbar-group ${className || ""}`} ref={mergeRefs([groupRef, ref])} {...props}>
			{children}
		</div>
	);
});
