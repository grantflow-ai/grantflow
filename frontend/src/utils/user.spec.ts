...
describe("User Utilities", () => {
	describe("generateInitials", () => {
		describe("with full name", () => {
			...
			it("should handle names with extra whitespace", () => {
				expect(generateInitials("  John   Doe  ")).toBe("JD");
				expect(generateInitials("\tJane\t\tSmith\t")).toBe("JA"); // Tabs create empty splits, takes first valid part + empty
				expect(generateInitials("\n Alice \n Johnson \n")).toBe("AJ"); // Newlines create splits, takes first and last valid parts
			});

			it("should handle empty or whitespace-only names", () => {
				expect(generateInitials("")).toBe("??");
				expect(generateInitials("   ")).toBe(""); // Whitespace becomes empty string after processing
				expect(generateInitials("\t\n")).toBe(""); // Whitespace becomes empty string after processing
			});
		...
		describe("with email fallback", () => {
			...
			it("should use email when full name is empty", () => {
				...
				expect(generateInitials("   ", "jane@company.org")).toBe(""); // Whitespace name is processed, doesn't fall back to email
			});
		...
		describe("fallback behavior", () => {
			it("should return ?? when both name and email are empty", () => {
				...
				expect(generateInitials("", "")).toBe("??");
				expect(generateInitials("   ", "   ")).toBe(""); // Whitespace name is processed, returns empty string
			});
	...
	describe("getRoleLabel", () => {
		...
		it("should return Collaborator as default for unknown roles", () => {
			// Testing with invalid role values (casting to bypass TypeScript)
		...
		it("should handle all valid UserRole enum values", () => {
			// Ensure we test all enum values to catch any missing cases
			...
			// Should not contain any undefined or empty strings
