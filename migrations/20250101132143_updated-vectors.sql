...
CREATE TABLE "grant_formats" (
  ...
  PRIMARY KEY ("id"),
  CONSTRAINT "grant_formats_funding_organization_id_fkey" FOREIGN KEY ("funding_organization_id") REFERENCES "funding_organizations" ("id") ON UPDATE NO ACTION ON DELETE CASCADE
);
-- Create index "ix_grant_formats_funding_organization_id" to table: "grant_formats"
CREATE INDEX "ix_grant_formats_funding_organization_id" ON "grant_formats" ("funding_organization_id");
-- Create index "uq_format_name_funding_org" to table: "grant_formats"
CREATE UNIQUE INDEX "uq_format_name_funding_org" ON "grant_formats" ("name", "funding_organization_id");
...
ALTER TABLE "application_files" DROP CONSTRAINT "application_files_application_id_fkey", ADD COLUMN "text_content" text NULL, ADD COLUMN "created_at" timestamptz NOT NULL DEFAULT now(), ADD COLUMN "updated_at" timestamptz NOT NULL, ADD
