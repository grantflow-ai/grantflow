...
export const TagElement = withRef<typeof PlateElement>(({ children, className, ...props }, ref) => {
	const { element } = props;
	...
	const content = readOnly && element.url ? <Link href={element.url as string}>{badge}</Link> : badge;

	return (
		<PlateElement
			ref={ref}
			className={cn(className, "m-0.5 inline-flex cursor-pointer select-none")}
			draggable
			{...props}
		>
			{content}
