...
def test_chunk_text_plain(mocker: MockFixture) -> None:
    ...
    mocker.patch("packages.shared_utils.src.chunking.get_splitter", return_value=mock_splitter)
...
def test_chunk_text_markdown(mocker: MockFixture) -> None:
    ...
    assert len(result) == 3
    assert [chunk["content"] for chunk in result] == ["# Header", "Paragraph 1", "Paragraph 2"]
    mock_splitter.chunks.assert_called_once_with(text)
