...
def _process_lines(lines: list[str]) -> list[str]:
    ...
    for i, line in enumerate(lines):
        ...
        _handle_regular_line(
            normalized_lines, current_list_items, normalized_line, i, lines
        )
...
def _handle_empty_line(
    normalized_lines: list[str], current_list_items: list[str]
) -> None:
    if current_list_items:
        normalized_lines.extend(current_list_items)
...
def _handle_header(
    normalized_lines: list[str], current_list_items: list[str], normalized_line: str
) -> None:
    if current_list_items:
        normalized_lines.extend(current_list_items)
        current_list_items.clear()
