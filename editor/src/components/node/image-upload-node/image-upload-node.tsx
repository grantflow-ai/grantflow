...
import "@/components/node/image-upload-node/image-upload-node.scss";
	...
	const uploadFile = async (file: File): Promise<string | null> => {
		...
		try {
			...
			if (!abortController.signal.aborted) {
				setFileItems((prev) =>
					prev.map((item) =>
						item.id === fileId ? { ...item, progress: 100, status: "success", url } : item,
					),
				);
			...
			if (!abortController.signal.aborted) {
				setFileItems((prev) =>
					prev.map((item) => (item.id === fileId ? { ...item, progress: 0, status: "error" } : item)),
				);
