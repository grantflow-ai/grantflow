...
from packages.shared_utils.src.exceptions import ExternalOperationError, UrlParsingError, ValidationError
...
class TestCrawlerPipeline:
    ...
    async def test_url_preparation_smoke(
        ...
        try:
            ...
            assert expected_content in html_content, f"Expected content '{expected_content}' not found in HTML"
            assert test_url in visited_urls, f"Test URL {test_url} not in visited URLs"

            logger.info("✓ URL preparation smoke test passed: %d chars retrieved", len(html_content))
    ...
    async def test_link_extraction_smoke(
        ...
        try:
            ...
            assert len(absolute_links) >= 2, f"Too few absolute links: {len(absolute_links)}"

            
            doc2_link = "https://example.com/relative/doc2.pdf"
            assert doc2_link in doc_links, f"Relative URL not properly converted: {doc2_link}"

            logger.info("✓ Link extraction smoke test passed: %d total links (%d docs, %d normal)", len(all_links), len(doc_links), len(normal_links))
    ...
    async def test_content_processing_smoke(
        ...
        try:
            ...
            assert len(text_content) >= 100, f"Text content too short: {len(text_content)} chars"
            ...
            logger.info("✓ Content processing smoke test passed: %d chars text, %d chars markdown", len(text_content), len(markdown_content))
    ...
    async def test_crawling_quality_assessment(
        ...
        try:
            ...
            assert text_content.strip(), "No text content extracted"
            assert len(text_content) >= 50, f"Text content too short: {len(text_content)} chars"
            ...
            if chunk_lengths:
                ...
                assert 100 <= avg_chunk_length <= 2500, f"Average chunk length suspicious: {avg_chunk_length}"
            ...
            alpha_chars = sum(1 for c in text_content if c.isalpha())
            alpha_ratio = alpha_chars / len(text_content) if text_content else 0
            assert alpha_ratio > 0.2, f"Low alphabetic character ratio: {alpha_ratio:.2f}"
    ...
    async def test_link_relevance_assessment(
        ...
        try:
            ...
            for link_data in relevant_links:
                ...
                assert len(link_data) == 4, "Each relevant link tuple should have 4 elements"
        ...
        try:
            ...
            async with TemporaryDirectory() as temp_dir:
                ...
                if downloaded_files:
                    ...
                    logger.info("✓ Document processing quality test passed: %d files processed", len(downloaded_files))
                else:
    ...
    async def test_comprehensive_crawling_pipeline(
        ...
        try:
            ...
            assert len(text_content) >= 50, f"Insufficient text content: {len(text_content)} chars"
            ...
            alpha_chars = sum(1 for c in text_content if c.isalpha())
            alpha_ratio = alpha_chars / len(text_content) if text_content else 0
            assert alpha_ratio > 0.15, f"Poor content quality - low alpha ratio: {alpha_ratio:.2f}"

            
            total_chunk_chars = sum(chunk_lengths)
            coverage_ratio = total_chunk_chars / len(text_content) if text_content else 0
            assert 0.5 <= coverage_ratio <= 2.0, f"Coverage ratio suspicious: {coverage_ratio}"

            
            unique_contents = set(chunk_contents)
            duplicate_ratio = 1 - (len(unique_contents) / len(chunk_contents))
            assert duplicate_ratio < 0.3, f"Too many duplicate chunks: {duplicate_ratio:.1%}"
            ...
            logger.exception("Unexpected error in comprehensive evaluation for %s", test_url)
            pytest.fail(f"Unexpected comprehensive evaluation error for {test_url}: {e}")
    ...
    async def test_content_semantic_quality(
        ...
        try:
            ...
            assert text_content.strip(), "No text content extracted"
            assert len(text_content) > 200, f"Content too short: {len(text_content)} chars"
            assert embeddings is not None, "No embeddings generated"

            
            content_lower = text_content.lower()
            research_keywords = ["research", "grant", "application", "project", "budget", "methodology"]
            found_keywords = [kw for kw in research_keywords if kw in content_lower]
            
            assert len(found_keywords) >= 4, f"Too few research-related keywords: {found_keywords}"

            
            words = text_content.split()
            assert len(words) >= 30, f"Too few words for semantic analysis: {len(words)}"

            
            alpha_chars = sum(1 for c in text_content if c.isalpha())
            alpha_ratio = alpha_chars / len(text_content) if text_content else 0
            assert alpha_ratio > 0.5, f"Low alphabetic character ratio for research content: {alpha_ratio:.2f}"

            
            lines = text_content.split('\n')
            non_empty_lines = [line for line in lines if line.strip()]
            assert len(non_empty_lines) >= 5, f"Poor content structure: {len(non_empty_lines)} lines"
