...
async def test_url_content_extraction(
    ...
    try:
        ...
        assert len(html_content) > 100, f"HTML content too short: {len(html_content)} chars"
        assert expected_content.lower() in html_content.lower(), f"Expected content '{expected_content}' not found"
...
async def test_link_extraction_comprehensive(logger: logging.Logger) -> None:
    ...
    try:
        ...
        for expected_link in expected_absolute_links:
            assert expected_link in link_set, f"Expected link not found: {expected_link}"
        
        
        assert "https://external.gov/regulations.pdf" in link_set, "External absolute URL not preserved"
        
        
        assert "https://grants.example.com/resources/templates.zip" in link_set, "Relative path not resolved correctly"
