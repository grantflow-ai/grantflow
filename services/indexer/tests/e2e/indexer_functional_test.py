...
@performance_test(execution_speed=ExecutionSpeed.SMOKE, domain=Domain.INDEXER, timeout=60)
...
@performance_test(execution_speed=ExecutionSpeed.SMOKE, domain=Domain.INDEXER, timeout=90)
@pytest.mark.parametrize("data_file", QUICK_TEST_FILES)
...
@performance_test(execution_speed=ExecutionSpeed.SMOKE, domain=Domain.INDEXER, timeout=120)
...
@performance_test(execution_speed=ExecutionSpeed.QUALITY, domain=Domain.INDEXER, timeout=180)
...
@performance_test(execution_speed=ExecutionSpeed.QUALITY, domain=Domain.AI_EVALUATION, timeout=240)
...
@performance_test(execution_speed=ExecutionSpeed.QUALITY, domain=Domain.INDEXER, timeout=300)
async def test_database_integration_quality(
    ...
    try:
        ...
        async with async_session_maker() as session:
            ...
            for vector in vectors:
                assert len(vector["embedding"]) == 384, f"Unexpected embedding dimension: {len(vector['embedding'])}"
...
@performance_test(execution_speed=ExecutionSpeed.E2E_FULL, domain=Domain.INDEXER, timeout=600)
