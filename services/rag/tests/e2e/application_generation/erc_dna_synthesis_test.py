...
async def test_generate_erc_application_for_dna_synthesis(
    ...
    logger.info("🧬 Generating ERC application for DNA synthesis research using scenario: %s", scenario.scenario_name)

    performance_context.start_stage("load_scenario_metadata")

    form_inputs = scenario.form_inputs

    assert "DNA synthesis" in form_inputs["background_context"], "Scenario should be about DNA synthesis"
    assert "C. elegans" in form_inputs["hypothesis"], "Scenario should mention C. elegans"
    ...
    logger.info(
        "✅ DNA synthesis ERC application generated successfully",
        extra={
...
async def test_validate_dna_synthesis_scenario_structure(
    logger: logging.Logger,
    performance_context: PerformanceTestContext,
) -> None:
    logger.info("🔍 Validating DNA synthesis scenario structure and metadata")
    ...
    assert metadata.name == "ERC PoC DNA Synthesis", f"Unexpected scenario name: {metadata.name}"
    assert metadata.researcher == "Asaf", f"Unexpected researcher: {metadata.researcher}"
    assert metadata.grant_type == "ERC Proof of Concept", f"Unexpected grant type: {metadata.grant_type}"
    assert metadata.research_domain == "DNA_synthesis", f"Unexpected research domain: {metadata.research_domain}"
    ...
    eca_objective = next((obj for obj in objectives if "ECA" in obj.get("title", "")), None)
    assert eca_objective is not None, "Should have an objective about ECA formation"
    ...
    assert "DNA synthesis" in background, "Background should mention DNA synthesis"
    assert "C. elegans" in hypothesis, "Hypothesis should mention C. elegans"
    ...
    assert len(source_files) > 0, "Should have source files for DNA synthesis research"
    ...
    logger.info(
        "✅ DNA synthesis scenario structure validation completed",
        extra={
            "scenario": scenario.scenario_name,
