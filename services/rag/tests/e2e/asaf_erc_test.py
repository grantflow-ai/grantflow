...
async def test_generate_erc_application_for_asaf(
    ...
    logger.info("Created grant application with ID: %s", grant_application_id)
    logger.info("Generating application text using the pipeline handler")
    ...
    assert len(text) > 5000, f"Generated text is too short: {len(text)} characters for a full ERC application"
    ...
    for keyword in expected_keywords:
        assert re.search(keyword, text.lower()), f"Expected keyword '{keyword}' not found in generated text"
    ...
    for objective in research_objectives:
        ...
        pattern = re.compile(
            r"(?i)" + re.escape(objective_title.lower().replace("in c. elegans", "")).replace(r"\ ", r"\s+")
        )
        assert pattern.search(text.lower()), f"Research objective '{objective_title}' not mentioned in the text"
    ...
    text_result_file = result_folder / f"full_text_result_text_asaf_{timestamp}.md"
    text_result_file.write_text(text)

    sections_result_file = result_folder / f"full_text_result_section_texts_asaf_{timestamp}.json"
    sections_result_file.write_bytes(serialize(section_texts))

    logger.info("Generated ERC application for Asaf with %d sections and %d characters", len(section_texts), len(text))
