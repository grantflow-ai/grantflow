...
async def test_failed_job_reset_to_processing(
    ...
    job_manager2: GrantApplicationJobManager[Any] = GrantApplicationJobManager(
        grant_application_id=test_application_with_template.id,
        parent_id=test_application_with_template.id,
        current_stage=GrantApplicationStageEnum.GENERATE_SECTIONS,
        pipeline_stages=list(GRANT_APPLICATION_STAGES_ORDER),
...
async def test_non_failed_job_not_reset(
    ...
    async with async_session_maker() as session, session.begin():
        ...
        job.current_stage = GrantApplicationStageEnum.EXTRACT_RELATIONSHIPS
        session.add(job)
    ...
    async with async_session_maker() as session:
        ...
        assert same_job.current_stage == GrantApplicationStageEnum.EXTRACT_RELATIONSHIPS
