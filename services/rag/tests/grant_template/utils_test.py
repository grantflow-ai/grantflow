...
def test_detect_cycle_complex() -> None:
    ...
    assert detect_cycle(graph, "a", set(), set())  # a -> b -> d -> g -> i -> j -> k -> e -> g ~keep
    assert detect_cycle(graph, "b", set(), set())  # b -> d -> g -> i -> j -> k -> e -> g ~keep
    assert detect_cycle(graph, "c", set(), set())  # c -> f -> h -> i -> j -> k -> e -> g -> i ~keep
    assert not detect_cycle(graph, "1", set(), set())  # 1 -> 2 ~keep
