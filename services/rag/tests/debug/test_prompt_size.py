...
def estimate_prompt_size(objectives: list[ResearchObjective]) -> dict:
    ...
    # Format objectives as in the actual code
    objectives_text = "\n\n".join([
        f"Objective {obj['number']}: {obj['title']}\nTasks: {obj['research_tasks']}"
        for obj in objectives
    ])
    
    # Mock data similar to real usage
    ...
    # Add mock RAG results (this can be quite large)
...
test_cases = [
    ...
    # Three objectives (similar to what fails)
    [
        {
            "number": i,
...
for objectives in test_cases:
    ...
    print(f"Objectives: {results['objectives_count']}")
    print(f"  - Objectives text: {results['objectives_text_length']:,} chars")
    print(f"  - Prompt template: {results['prompt_template_length']:,} chars")
    print(f"  - Full prompt: {results['full_prompt_length']:,} chars")
...
print("\nAnalysis:")
print("- Single objective prompts are manageable")
print("- As objectives increase, prompt size grows significantly")
print("- RAG results add substantial size to the prompt")
print("- Large prompts may cause API timeouts or connection issues")