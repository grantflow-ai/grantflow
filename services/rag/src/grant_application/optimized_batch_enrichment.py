...
CLAUDE_CONTEXT_LIMIT: Final[int] = 200_000  # Claude 3.5 Sonnet context limit
SAFETY_MARGIN: Final[float] = 0.85  # Use 85% of context to be safe
OPTIMAL_BATCH_SIZE: Final[int] = 5  # Based on baseline: 5 objectives worked well
MAX_RETRIEVAL_TOKENS: Final[int] = 8000  # Limit retrieval size for efficiency
...
async def calculate_optimal_batching(
    ...
    # Estimate tokens for shared content (retrieval + base prompt)
    base_tokens = estimate_prompt_tokens(retrieval_content) + 2000  # Base prompt overhead
    available_tokens = int(CLAUDE_CONTEXT_LIMIT * SAFETY_MARGIN) - base_tokens
    
    # Estimate tokens per objective (title + tasks + output)
    sample_obj = objectives[0]
    obj_text = f"Objective {sample_obj['number']}: {sample_obj['title']}\nTasks: {sample_obj['research_tasks']}"
    tokens_per_obj = estimate_prompt_tokens(obj_text) * 2.5  # Include expected output
    
    # Calculate objectives per batch
...
async def create_optimized_retrieval_context(
    *, 
    ...
    """
    # Combine all objective contexts for retrieval
    combined_context = "\n\n".join([
    ...
    # Extract key terms from objectives for additional queries
    for obj in research_objectives[:3]:  # Limit to first 3 to avoid too many queries
        ...
        if key_terms:
            search_queries.append(" ".join(key_terms[:2]))  # Max 2 terms per objective
    ...
    retrieval_result = await retrieve_documents(
        ...
        search_queries=search_queries[:8],  # Limit total queries for efficiency
...
async def handle_optimized_batch_enrichment(
    *,
    ...
    # Step 1: Single optimized retrieval for all objectives
    ...
    # Step 3: Process batches optimally
    ...
    if len(objective_batches) == 1:
        ...
        # Two batches - process in parallel for maximum speed
        ...
        # Multiple batches - process first two in parallel, rest sequentially
        logger.info(
        ...
        # Process first two batches in parallel
        parallel_tasks = [
            handle_batch_enrich_objectives(
                application_id=application_id,
                grant_section=grant_section,
...
async def estimate_performance_improvement(
    ...
    baseline_seconds: float = 488.0,  # From our baseline test
) -> dict[str, float]:
    """
    ...
    """
    # Baseline metrics (from our test)
    baseline_batch_time = baseline_seconds  # 488s for 5 objectives
    objectives_per_batch = 5
    time_per_objective_baseline = baseline_batch_time / objectives_per_batch  # ~97.6s per objective
    
    # Optimization estimates
    retrieval_overhead_savings = 0.15  # 15% savings from single retrieval
    batching_efficiency = 0.10  # 10% savings from optimal batching
    parallel_processing_boost = min(0.20, objectives_count * 0.04)  # Up to 20% from parallelization
