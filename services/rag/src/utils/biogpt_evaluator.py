...
class BioGPTEvaluator:
    ...
    def load_model(self) -> bool:
        ...
        try:
            ...
            logger.info(f"Loading BioGPT model: {BIOGPT_MODEL_NAME}")
            ...
            logger.info(f"✅ BioGPT model loaded successfully in {self._load_time:.2f}s")
            return True
            
        except Exception as e:
            logger.error(f"❌ {MODEL_LOAD_ERROR_MSG}: {str(e)}")
            self._is_loaded = False
            return False
    ...
    def calculate_perplexity(self, text: str, max_length: int = DEFAULT_MAX_LENGTH) -> float:
        ...
        try:
            ...
            logger.error(f"❌ {INFERENCE_ERROR_MSG}: {str(e)}")
            raise BackendError(f"Failed to calculate perplexity: {str(e)}") from e
    ...
    def evaluate_text_quality(self, text: str) -> dict[str, Any]:
        ...
        try:
            ...
            logger.error(f"Failed to evaluate text quality: {str(e)}")
            raise BackendError(f"Text quality evaluation failed: {str(e)}") from e
    ...
    def batch_evaluate_texts(self, texts: list[str]) -> list[dict[str, Any]]:
        ...
        for i, text in enumerate(texts):
            try:
                ...
                logger.error(f"Failed to evaluate text {i+1}: {str(e)}")
                results.append({
                    "text_index": i,
                    "error": str(e),
...
_biogpt_evaluator: Optional[BioGPTEvaluator] = None


def get_biogpt_evaluator(model_path: Optional[str] = None) -> BioGPTEvaluator:
    """
...
def load_biogpt_model(model_path: Optional[str] = None) -> bool:
    """
