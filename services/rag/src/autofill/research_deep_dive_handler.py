...
class ResearchDeepDiveHandler(BaseAutofillHandler):
    ...
    async def _generate_content(self, request: AutofillRequestDTO, application: dict[str, Any]) -> dict[str, Any]:
        ...
        target_fields = (
            [request["field_name"]] if request.get("field_name")
        ...
        self.logger.info("Starting research deep dive generation",
        ...
        self.logger.debug("Retrieved documents for deep dive",
                         application_id=request["parent_id"],
        ...
        for field_name in target_fields:
            if field_name in existing_answers and existing_answers[field_name] and existing_answers[field_name].strip():
                ...
                self.logger.debug("Skipping field with existing content",
            ...
            self.logger.debug("Generated answer for field",
                            field_name=field_name,
                            application_id=request["parent_id"],
                            answer_length=len(answer))

        self.logger.info("Research deep dive generation completed",
                        application_id=request["parent_id"],
...
async def generate_research_answers(request: AutofillRequestDTO, logger: logging.Logger) -> dict[str, Any]:
    """Generate research deep dive answers"""
