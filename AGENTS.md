...
Generated on 2025-09-15 15:25:59
...
# Or start specific services
task service:backend:dev  # API service on port 8000
task service:indexer:dev  # Document processing
...
Follow strict workflow: task lint:all → task test → task build. Never run raw commands. Check Taskfile.yaml for available tasks. Use 'task gh:deploy:{service}' for deployments.
...
Services (backend, indexer, crawler, rag, scraper) must be independently deployable. Use packages/db for shared models, packages/shared_utils for utilities. Frontend at /frontend, CRDT server at /crdt. Deploy via `task gh:deploy:{service}` commands.
