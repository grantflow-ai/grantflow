"""Tests for grant alert email notifications."""
...
class TestGrantAlertEmails:
    ...
    async def test_send_grant_alert_email_success(
        ...
        result = await send_grant_alert_email(notification_data)
    ...
    async def test_send_grant_alert_single_grant(
        ...
        # Verify singular form in subject
        mock_send_resend.assert_called_once()
        args = mock_send_resend.call_args.args
        assert args[1] == "🎯 1 New Grant Available"  # Singular
    ...
    async def test_main_function_routes_grant_alerts(
        ...
        # Create CloudEvent with grant alert data
        grant_alert_data = {
            "email": "user@example.com",
            "template_type": "grant_alert",
            "template_data": {
    ...
    async def test_main_function_routes_application_emails(
        ...
        # Create CloudEvent with application data (no template_type)
        application_data = {
